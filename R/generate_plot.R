...
generate_plot <- function(df, file_name, polar, filetype, color = "black", background_color = "white") {
  ...
  if (polar == TRUE) {
    ...
    plot <- df %>%
      ...
      ggplot2::theme_void() +
      ggplot2::theme(
        panel.background = element_rect(fill = background_color, debug=TRUE),
        plot.background = element_rect(fill = background_color, debug=TRUE)
        ) +
      ggplot2::coord_fixed()
  }
  ggplot2::ggsave(plot, filename = paste0(IMG_PATH, file_name), device = filetype)
  print("image saved...")
}
